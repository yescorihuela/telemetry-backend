class EventService
  def self.get_events_by_date_device(trip_id, datetime, license_plate = '')  
    EventRoute.select("events.id, license_plate, device_serial_number, ST_X(location::geometry) as latitude, ST_Y(location::geometry) as longitude, occurred_at, events.description")
